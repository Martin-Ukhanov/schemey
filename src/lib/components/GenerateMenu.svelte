...
<menu
	class="fixed bottom-4 left-4 right-4 h-64 p-4 flex gap-4 border-3 rounded-lg bg-white border-black"
>
	...
	<div class="flex-1 flex -mx-2">
		{#each colorSchemes[colorSchemeIndex] as color, index (color.id)}
			<div
				class="w-full p-8 mx-2 flex flex-col justify-between items-center border-3 rounded-lg border-black transition-colors duration-150"
	...
	{#if colorSchemes[colorSchemeIndex].length < MAX_COLOR_SCHEME_SIZE}
		<button class="button" on:click={addColor}>
			<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-8 fill-black">
